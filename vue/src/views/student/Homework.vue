  ...
  <div>
    ...
    <el-table :data="list" style="margin-top:10px">
      ...
      <el-table-column label="提交">
        <template #default="scope">
          <el-button @click="openSubmit(scope.row)">完成作业</el-button>
        </template>
      </el-table-column>
    </el-table>
...
const loadCourses = async () => {
  const r = await request.get("/api/student/courses", { params: { studentId: user.userId || user.id } });
...
const load = async () => {
  const r = await request.get("/api/student/homework", {
...
const submit = async () => {
  ...
  const res = await request.post("/api/homework/submit", {
    homeworkId: current.value.homework.id,
    studentId: user.userId || user.id,
    submitContent: JSON.stringify(payload)
  });
